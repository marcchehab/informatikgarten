...
var callouts = function callouts(providedConfig) {
  ...
  return function (tree) {
    unistUtilVisit.visit(tree, function (node, index, parent) {
       ...
       */
      unistUtilVisit.visit(node, "text", function (node, index, parent) {
        var result = [];
        var start = 0;
        find.lastIndex = 0;
       ...
       */
      unistUtilVisit.visit(node, "heading", function (node) {
        var heading = node;
        heading.data = {
          hProperties: {
      ...
      var parsedSvg;
      if (entry && entry.svg) {
        parsedSvg = svgParser.parse(entry.svg);
      }
